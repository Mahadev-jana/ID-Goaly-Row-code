...
const Sidebar = ({ open, closeSideBar }) => {
	...
	const linkToSubscribe = () => {
		window.location.replace('http://103.77.79.44:8900/lp/portal?service=goaly&package=weekly');
	}
	const linkToUnsubscribe = () => {
		const payLoad = new FormData();
		payLoad.append('msisdn', getUserDetails.phone_no);
		axios.get(`https://cms-id.goaly.mobi/subscription/unsub?msisdn=${getUserDetails().phone_no}`,payLoad)
			.then((res) => {
				setIsResponse(res);
